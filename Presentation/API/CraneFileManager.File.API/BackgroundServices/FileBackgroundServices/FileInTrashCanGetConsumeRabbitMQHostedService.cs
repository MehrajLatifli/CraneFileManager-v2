...
{
    public class FileInTrashCanGetConsumeRabbitMQHostedService<T> : BackgroundService, IDisposable where T : FileEventType
    {
        ...
        public FileInTrashCanGetConsumeRabbitMQHostedService(
            ILoggerFactory loggerFactory,
            IServiceScopeFactory serviceScopeFactory,
            IConfiguration configuration)
        {
            _logger = loggerFactory.CreateLogger<FileInTrashCanGetConsumeRabbitMQHostedService<T>>();
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _channel.ExchangeDeclare(exchange: "FileInTrashCanGet_exchange", type: ExchangeType.Direct, autoDelete: false);
            _channel.QueueDeclare("FileInTrashCanGet_queue", durable: true, exclusive: false, autoDelete: false);
            _channel.QueueBind("FileInTrashCanGet_queue", "FileInTrashCanGet_exchange", "FileInTrashCanGet_notification");
            ...
            consumer.Received += async (model, eventArgs) =>
            {
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    if (fileEvent.Eventtype == "FileInTrashCanGet_notification")
                    {
                        ...
                        if (fileEvent.IsIdentity)
                        {
                            ...
                            if (!string.IsNullOrEmpty(CurrentUser))
                            {
                               ...
                               await cacheServiceFileDTOforGetandGetAll.DeleteAllFilesInTrashCanByUser(CurrentUser);
                               await fileService.ViewFilesInTrashCan(CurrentUser);

                               await  cacheServiceFileDTOforGetandGetAll.GetAllFilesInTrashCanByUser(CurrentUser);
            ...
            _channel.BasicConsume("FileInTrashCanGet_queue", false, consumer);
